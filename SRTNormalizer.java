...
public class SRTNormalizer {
    public static void main(String[] args) {
    	...
    	if(args.length > 1) {
			...
			file = new File(args[0]);
            in = new BufferedReader(new FileReader(file));
            output = file.getAbsolutePath().substring(0, file.getAbsolutePath().length()-4) + "_normalized.srt";
		} else {
			throw new Exception("Usage:\njava SRTNormalizer [input.srt] [output.srt]\njava SRTNormalizer [input.srt]");
		}
		...
		System.out.println("Subtitle successfully normalized.");
		System.out.println("The normalized subtitle is exported as " + output + ".");

	} catch (StringIndexOutOfBoundsException alreadyConverted) {
		System.out.println("SRT file has already been normalized.");
